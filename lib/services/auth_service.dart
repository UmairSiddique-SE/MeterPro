...
class AuthService {
  ...
  Future<User?> signUp({required String email, required String password}) async {
    ...
    if (user == null) throw StateError('Firebase did not return the newly created account.');
    return user;
  }
  ...
  Future<AuthSignInResult> signIn({required String email, required String password}) async {
  ...
  Future<void> requestOtp({String? email, String? name}) async {
    ...
    if (lastSent is Timestamp &&
      ...
      throw StateError('Please wait 60 seconds before requesting another code.');
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      throw StateError(
        'Email service error (' + response.statusCode.toString() + ')' +
  ...
  Future<void> verifyOtp(String code) async {
    ...
    final user = _auth.currentUser;
    if (user == null) throw StateError('Please sign in before verifying your email.');
    ...
    if (!verified) {
      throw StateError('Invalid or expired verification code. Please request a new code.');
    }
  }
  ...
  Future<void> updateProfileName(String name) async {
    ...
    await _firestore.collection('users').doc(user.uid).update({'displayName': name.trim()});
  }
  ...
  Future<void> deleteAccount() async {
    ...
    for (final meter in meters.docs) batch.delete(meter.reference);
  ...
  static String messageFor(FirebaseAuthException e) {
    switch (e.code) {
      ...
      case 'weak-password': return 'Password is too weak (min. 8 characters).';
